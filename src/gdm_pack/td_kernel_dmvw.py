...
class TDKernelDMVW(object):
    ...
    def calculate_maps(self):
        ...
        for i in range(0, self.get_measurements_length()):
            ...
            cell_grid_x = self.cell_grid_x[low_index_x:high_index_x+1, low_index_y:high_index_y+1]
            cell_grid_y = self.cell_grid_y[low_index_x:high_index_x+1, low_index_y:high_index_y+1]
            ...
            self.importance_weight_map[low_index_x:high_index_x+1, low_index_y:high_index_y+1] += omega

            self.importance_weight_concentration_map[low_index_x:high_index_x+1, low_index_y:high_index_y+1] += omega * self.measurement_concentrations[i]
        ...
        np.nan_to_num(self.mean_map,copy=False)
        ...
        for i in range(0, self.get_measurements_length()):
            ...
            min_x = int(np.round((self.measurement_positions_x[i] - self.min_x) / self.cell_size, 0))
            min_y = int(np.round((self.measurement_positions_y[i] - self.min_y) / self.cell_size, 0))
            ...
            # V
            variance_map_temp[low_index_x:high_index_x+1, low_index_y:high_index_y+1] += self.norm_fact * np.exp(-(distance_matrix ** 2) * self.exp_fact) * variance_factor
